...
The strongest evidence for this product is not macroeconomic, it is benchmark evidence, and it is months old rather than four years old. [Frontier models that score above 80% on SWE-Bench Verified top out around 59 on SWE Atlas Refactoring, a 70-task suite drawn from 10 production repositories across 6 languages](https://labs.scale.com/leaderboard/sweatlas-refactoring), and the field has been climbing that specific number for months rather than treating it as solved. Issue resolution is close to solved. Behaviour-preserving restructuring across many files is not.
